...
const Header = () => {
    const { toggleSidebar, notifications, user, logout, theme, currentShift, fetchNotifications, syncStatus, lastSync } = useStore();
        ...
        <header className="h-16 px-6 glass flex items-center justify-between sticky top-0 z-40 border-b border-white/5">
            ...
            <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className={`hidden md:flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${syncStatus === 'synced'
                        ? 'bg-blue-500/10 border-blue-500/20 text-blue-400'
                        : syncStatus === 'syncing'
                            ? 'bg-amber-500/10 border-amber-500/20 text-amber-500 animate-pulse'
                            : 'bg-red-500/10 border-red-500/20 text-red-400'}`}
                        title={lastSync ? `Last Synced: ${new Date(lastSync).toLocaleString()}` : 'Never Synced'}>
                        <div className={`w-1.5 h-1.5 rounded-full ${syncStatus === 'synced' ? 'bg-blue-400' : syncStatus === 'syncing' ? 'bg-amber-500' : 'bg-red-400'}`} />
                        {syncStatus}
                    </div>
                </div>
            </div>
