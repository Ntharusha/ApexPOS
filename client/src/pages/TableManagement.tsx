...
const TableManagement = () => {
    ...
    const handleOpenOrder = async (tableId: string) => {
        try {
            const res = await fetch('http://localhost:5000/api/hospitality/orders', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ tableId, items: [], cashierName: user?.name || 'Staff' })
            });
            if (res.ok) {
                fetchTables();
    ...
    const handleCheckout = async (orderId: string) => {
        if (!orderId) {
            alert('No active order to checkout!');
            return;
        }
        try {
            const res = await fetch(`http://localhost:5000/api/hospitality/orders/${orderId}/pay`, {
                method: 'POST',
            });
            if (res.ok) {
                fetchTables();
                setSelectedTable(null);
                alert('Bill cleared and table is now Available!');
            }
        } catch (error) {
                            ...
                            <div className="p-8 flex-1 overflow-y-auto custom-scrollbar">
                                {selectedTable.status === 'Available' ? (
                                    <div className="space-y-6">
                                        ...
                                        <button
                                            onClick={() => handleOpenOrder(selectedTable._id)}
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="p-6 bg-white/5 border border-white/10 rounded-3xl">
                                            ...
                                            <div className="space-y-3 max-h-[200px] overflow-y-auto custom-scrollbar pr-2">
                                                {selectedTable.currentOrder?.items && selectedTable.currentOrder.items.length > 0 ? (
                                                    selectedTable.currentOrder.items.map((item: any, i: number) => (
                                                        <div key={i} className="flex justify-between items-center bg-white/5 p-3 rounded-xl border border-white/5">
                                                            <div className="flex items-center gap-3">
                                                                <span className="w-8 h-8 bg-primary/20 text-primary rounded-lg flex items-center justify-center font-black text-xs">
                                                                    {item.quantity}x
                                                                </span>
                                                    ...
                                                    <p className="text-gray-500 italic text-sm text-center py-4">No items added to order yet...</p>
                                                )}
                                            </div>
                                            ...
                                            {selectedTable.currentOrder?.items?.length > 0 && (
                                                <div className="mt-4 pt-4 border-t border-white/10 flex justify-between items-center">
                                                    ...
                                                    <span className="text-xl font-black text-white font-mono">
                                                        LKR {selectedTable.currentOrder.items.reduce((sum: number, item: any) => sum + (item.price * item.quantity), 0).toLocaleString()}
                                                    </span>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <button onClick={() => alert('Printing Kitchen Order Ticket...')} className="bg-white/5 hover:bg-white/10 text-white p-5 rounded-3xl font-black transition-all flex flex-col items-center gap-2 border border-white/10">
                                                <Printer size={20} className="text-blue-400" />
                                                <span className="text-xs uppercase tracking-widest">Print KOT</span>
                                            </button>
                                            <button onClick={() => alert('Select a new table to move to...')} className="bg-white/5 hover:bg-white/10 text-white p-5 rounded-3xl font-black transition-all flex flex-col items-center gap-2 border border-white/10">
                                                <Users size={20} className="text-amber-400" />
                                                <span className="text-xs uppercase tracking-widest">Move Table</span>
                                            </button>
                                            <button onClick={() => handleCheckout(selectedTable.currentOrder?._id || selectedTable.currentOrder)} className="bg-primary hover:opacity-90 text-white p-5 rounded-3xl font-black transition-all flex flex-col items-center gap-2 shadow-lg shadow-primary/20">
                                                <Printer size={20} />
