...
7. [Phase 4 — Ansible (Server Configuration)](#7-phase-4--ansible-server-configuration)
...
└───────────────┬───────────────────────────────┬─────────────────────────┘
    ...
    ┌──────────────┐    ┌──────────────┐    ┌──────────────────┐
    │  Terraform   │    │   Ansible    │    │  CloudWatch Free │
    │  (Free OSS)  │    │  (Free OSS)  │    │  10 Alarms       │
    └──────────────┘    └──────────────┘    │  5GB Logs        │
                                            └──────────────────┘
```
...
## 7. Phase 4 — Ansible (Server Configuration)
...
│   └── hosts.yml
├── playbooks/
│   ├── setup-all.yml          # Master playbook
│   ├── jenkins-setup.yml
│   ├── k3s-config.yml
│   └── monitoring-agent.yml
└── templates/
    ├── jenkins-nginx.conf.j2
    └── cloudwatch-config.json.j2
```

### 7.1 Ansible Config (`ansible/ansible.cfg`)
```ini
[defaults]
inventory = inventory/hosts.yml
remote_user = ubuntu
private_key_file = ~/.ssh/apexpos-key.pem
...
### 7.2 Inventory (`ansible/inventory/hosts.yml`)
```yaml
all:
  hosts:
    apexpos-server:
      ...
      ansible_user: ubuntu
      ansible_ssh_private_key_file: ~/.ssh/apexpos-key.pem
  children:
    jenkins:
      hosts:
        apexpos-server:
...
### 7.3 Jenkins Setup Playbook (`ansible/playbooks/jenkins-setup.yml`)
```yaml
---
- name: Setup Jenkins on Free-Tier EC2
  hosts: jenkins
  become: yes
  vars:
    jenkins_port: 8080
    node_version: "20"
    java_heap: "256m"  # Low memory for t2.micro
  ...
  tasks:
    ...
    - name: Install Java 17 (required by Jenkins)
      apt:
        name: openjdk-17-jre-headless  # headless = smaller footprint
        state: present
    ...
    - name: Configure Jenkins memory limits (256MB for free tier)
      lineinfile:
        path: /etc/default/jenkins
        regexp: '^JAVA_ARGS='
        line: 'JAVA_ARGS="-Xmx{{ java_heap }} -Xms128m -Djava.awt.headless=true"'
    ...
    - name: Install AWS CLI v2
      shell: |
        ...
        cd /tmp && unzip -o awscliv2.zip && ./aws/install --update
      args:
    ...
    - name: Show Jenkins initial password
      debug:
        msg: "Jenkins initial admin password: {{ jenkins_password.stdout | default('Already configured') }}"
...
### 7.4 k3s Configuration Playbook (`ansible/playbooks/k3s-config.yml`)
```yaml
---
- name: Configure k3s for ApexPOS
  hosts: k3s
  become: yes
  ...
  tasks:
    ...
    - name: Create apexpos namespace
      command: k3s kubectl create namespace apexpos
      ignore_errors: yes  # Ignore if already exists

    - name: Install Nginx Ingress Controller (lighter than Traefik for free tier)
      shell: |
        k3s kubectl apply -f https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.9.0/deploy/static/provider/baremetal/deploy.yaml
      args:
        creates: /tmp/.ingress-installed
...
### 7.5 CloudWatch Agent Playbook (`ansible/playbooks/monitoring-agent.yml`)
```yaml
---
- name: Install CloudWatch Agent (Free Tier)
  hosts: all
  become: yes
  ...
  tasks:
    ...
    - name: Deploy CloudWatch config (free tier limits)
      template:
...
### 7.6 Master Playbook (`ansible/playbooks/setup-all.yml`)
```yaml
---
- name: Complete ApexPOS Server Setup
  import_playbook: jenkins-setup.yml

- name: Configure k3s
  import_playbook: k3s-config.yml
...
### 7.7 Run Ansible
```bash
# Set the server IP from Terraform output
export SERVER_IP=$(cd terraform && terraform output -raw server_public_ip)
...
ansible-playbook playbooks/k3s-config.yml
ansible-playbook playbooks/monitoring-agent.yml
```
...
### 9.2 Jenkins Setup Steps (After Ansible)
1. **Access Jenkins**: `http://<EC2-IP>:8080`
2. **Enter Initial Password** (from Ansible output or `sudo cat /var/lib/jenkins/secrets/initialAdminPassword`)
...
### 11.3 CloudWatch Agent Config (`ansible/templates/cloudwatch-config.json.j2`)
```json
{
  "agent": {
...
| **Phase 4** | Ansible (Jenkins, k3s, Agent) | 1–2 days |
...
| k3s / Jenkins / Terraform / Ansible | **$0** (open source) |
...
□ 7. Run: ansible-playbook playbooks/setup-all.yml
□ 8. Build & push Docker image
□ 9. Deploy K8s manifests to k3s
